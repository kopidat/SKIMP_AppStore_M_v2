			...
			if (listData.length == 0) {
				...
				listData.forEach((item, idx) => {
					...
					try {
						...
						// 개행문자 치환 및 JSON.parse() 를 이용할 때는 key와 value가 모두 쌍따옴표로 감싸져야 해서 '를 "로 치환
						var _msg = item.message;
						var _replaceMsg = _msg.replace(/\r\n|\n|\r/gm, "<br>").replace(/'/gm, "\"");
					    var json = JSON.parse(_replaceMsg);
...
//						tempBody = JSON.parse(item.message).body	;
					...
					listHtml += '<dl>';
					listHtml += '	<dt seqno="'+item.seqno+'" class="cursor">';
				...
				if(seqNo != null && seqNo != undefined && seqNo != '') {
				    $('.notice-list > dl > dd').slideUp();
                    $('dt[seqno='+seqNo+']').next().slideDown();
                    $('dt[seqno='+seqNo+']').addClass('active').parent().siblings('dl').find('dt.active').removeClass('active');
				}
