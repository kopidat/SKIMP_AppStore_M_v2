...
//			$('.full_loding').addClass('none');
			...
			if(rst.status == "1000"){
				...
				//21.10.21 cyj 연계 앱에서 PW오류로 인증 절차를 정상으로 밟지 못하는 부분이 있어 해당 부분을 서버에서 가져온 값으로 저장
				M.execute('exWNSetUserInfo', {
					userId : encIdNative,
					encPwd : rst.encPwd,//encPwNative,
					token : rst.token,
				});
...
var getUserInfo = function() {
	MNet.httpSend({
		path : "skimp/common/api/SKIMP-0004",
		sendData : {
			userId : MData.storage('encData').userId,
		},
		callback : function(rst) {
			//앱스토어 전문에서 사용할 usrInfo 저장
			MData.storage('uinfo', {
				name : rst.userInfoDto.emp_nm
				, division : rst.userInfoDto.division_nm
				, empNo : rst.userInfoDto.emp_no
				, siteCd : rst.userInfoDto.siteSmallClsCode
				, siteNm : rst.userInfoDto.siteSmallClsName
			});
		},
		errCallback : function(errCd, errMsg) {
			console.log(errCd, errMsg);
