...
public class HttpCustomNetworkManager extends AsyncHttpNetwork {
	...
	public synchronized void requestData(final String trCode, final DataHandler sb, final String otherInfos, final Object objSender, NetReqOptions netReqOptions) {
		PLog.e(CLASS_TAG, "requestData(final String trCode, final DataHandler sb, final String otherInfos, final Object objSender, NetReqOptions netReqOptions)");
		...
		new Thread() {
            public void run() {
            	...
            	try {
					...
					// Making all packet data - no "head", "body"
					sendStr = setRequestData(sh, sb, finalNetReqOptions, packetSeq);
					PLog.e(CLASS_TAG, "no head body sendStr = " + sendStr);
...
//						requestAndReceiveData(finalNetReqOptions.retargetUrl, httpSh, params, "");
	...
	public JSONObject setRequestHttpHeader(String trCode, NetReqOptions netReqOptions, int packetSeq) {
		PLog.e(CLASS_TAG, "setRequestHttpHeader(String trCode, NetReqOptions netReqOptions, int packetSeq)");
...
//			    [requestHeaders setObject:json_type   forKey:@"Accept"  ];
	...
	public JSONObject setRequestHeader(String otherInfos, int packetSeq, NetReqOptions netReqOptions) {
		PLog.e(CLASS_TAG, "setRequestHeader(String otherInfos, int packetSeq, NetReqOptions netReqOptions)");
...
//			 subHeadJson.put("callback_function", otherInfos);
	...
	public String setRequestData(JSONObject sh, DataHandler sb, NetReqOptions netReqOptions, int packetSeq) {
		PLog.e(CLASS_TAG, "setRequestData(JSONObject sh, DataHandler sb, NetReqOptions netReqOptions, int packetSeq)");
		return setNoHeadBodyRequestData(sb, packetSeq);
	}
	...
	public JSONObject getResponseHttpHeader(Header hd[], int packetSeq){
		PLog.e(CLASS_TAG, "getResponseHttpHeader(Header hd[], int packetSeq)");
...
//		String content_length = null;
	...
	public synchronized void getResponseData(Header hd[], String rd, int packetSeq) {
		PLog.e(CLASS_TAG, "getResponseData(Header hd[], String rd, int packetSeq)");
		getNoHeadBodyResponseData(hd, rd, packetSeq);
	}
	...
	private String setNoHeadBodyRequestData(DataHandler sb, int packetSeq) {
		PLog.e(CLASS_TAG, "setNoHeadBodyRequestData(DataHandler sb, int packetSeq)");
		String sendData = null;
		try{
			if (sb != null) {
				JSONObject sendJSON = new JSONObject(sb.toString());
			...
			try {
				if (sb != null) {
					JSONArray sendJSONArray = new JSONArray(sb.toString());
					PLog.i(CLASS_TAG, "// Request Data [" + sb.toString() + "]");
					sendData = sendJSONArray.toString();
				}
			} catch(Exception i) {
				handlingError(LibDefinitions.errstatus.ERROR_INTERNAL_NETWORK,
						Utils.getDynamicID(commHandle.getApplicationContext(), "string", "mp_addon_net_error_msg_internal_network2"),
	...
	private void getNoHeadBodyResponseData(Header hd[], String rd, int packetSeq) {
		PLog.e(CLASS_TAG, "getNoHeadBodyResponseData(Header hd[], String rd, int packetSeq)");
		PLog.e(CLASS_TAG, "hd = " + hd);
		PLog.e(CLASS_TAG, "rd = " + rd);
		PLog.e(CLASS_TAG, "packetSeq = " + packetSeq);
		String strTrCode = (String) HttpPacketManager.getInstance().getTrCode(packetSeq);
		...
		AbstractActivity selectedActivity = (AbstractActivity)HttpPacketManager.getInstance().getCallerObject(packetSeq);

		if (selectedActivity != null) {
			// Processing packet received informations
			// Call back function name
			String callBackFunc = (String)HttpPacketManager.getInstance().getCallBackFunctionName(packetSeq);

			// Requested network options
			NetReqOptions networkOptions = (NetReqOptions)HttpPacketManager.getInstance().getNetworkOptions(packetSeq);

			// Requested progressDialog
			// 요청시 생성한 프로그래스 다이얼로그(인디게이터)를 제거한다.
			HttpPacketManager.getInstance().removeProgressDialog(packetSeq);
			selectedActivity.responseData(IActivityNetworkable.RESPONSE_DATA_TYPE_HTTP, strTrCode, callBackFunc, rd, networkOptions);
		} else {
			HttpPacketManager.getInstance().removeAllProgressDialog();
			AbstractActivity topAct = (AbstractActivity)ActivityHistoryManager.getInstance().getTopActivity();
			// 요청에 대한 결과가 실패인 경우 에러처리를 한다.
			handlingError(topAct, strTrCode, Integer.valueOf("-1"), "error", null);
		}
