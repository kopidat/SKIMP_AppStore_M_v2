...
import skimp.member.store.LogoutNotiClickActivity;
import skimp.member.store.R;
import skimp.member.store.common.Const;
import skimp.member.store.nativeex.AES256Util;
import skimp.member.store.nativeex.MDM;
import skimp.member.store.nativeex.NativeBase;
...
import skimp.member.store.patternlock.util.PatternLockUtils;
import skimp.member.store.pinlock.activity.PinActivity;
import skimp.member.store.provider.AuthContentProvider;
import skimp.member.store.provider.DBManager;
import skimp.member.store.manager.RootCheck;
...
//    	}
	...
	*/
	...
	public void exWNRootCheck() {
		if(new RootCheck().isDeviceRooted(callerObject))
		{
	...
	public void exWNVaildApp() {
		...
		try {
			...
			PLog.e(TAG, "dbAppHashKey = " + dbAppHashKey);
	...
	public void exCallExit(){
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
			((Activity)callerObject).finishAffinity();
		}
	}

	/**
	 * 서버 실시간 등록대상 단말 등록 API.xlsx
	 * 인터페이스명	등록대상단말 등록/수정
	 * 경로	/agent/setRegTargetDeviceInfo.do
	 * 상용 서버	(예정)
	 * 테스트 서버	(예정)
	 * 설명	외부의 앱에서 단말의 정보(안드로이드는 MAC/IMEI/SSAID, IOS는 시리얼넘버)를 넘기면 저장할 수 있는 API 제공
	 *
	 * MDM(SSM) API 호출을 위한 정보전달 확장함수
	 * @return
	 */
	public JSONObject exWNGetSSMInfo() {
		JSONObject resultJson = new JSONObject();
		try {
			resultJson.put("userId", "");
			String androidId = Settings.Secure.getString(callerObject.getContentResolver(), Settings.Secure.ANDROID_ID).toUpperCase();
			resultJson.put("mac", androidId);
			resultJson.put("imei", "");
			resultJson.put("iosDeviceSerialNum", "");
			resultJson.put("companyAsset", "0");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String exWNMDMInstalledYN() {
		boolean isInstalled = MDM.getInstance().isInstalled();
		return isInstalled ? "Y" : "N";
	}

	public int exWNCheckMDM() {
		PLog.i(TAG, "exWNCheckMDM()");
		int mdmResult = MDM.getInstance().checkMDM(callerObject);
		PLog.d(TAG, "mdmResult = " + mdmResult);
		return mdmResult;
	}

	public void exWNGoMDMInstallPage() {
		MDM.getInstance().goInstallPage(callerObject);
	}

	public int exWNMDMLogin() {
		PLog.i(TAG, "exWNMDMLogin()");
		return MDM.getInstance().login();
	}

	public int exWNMDMLogout() {
		PLog.i(TAG, "exWNMDMLogout()");
		return MDM.getInstance().logout();
	}
	...
	public void exWNRegisterLogoutNoti() {
		...
		NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(callerObject.getApplicationContext(), ExtendApplication.getLogoutNotiChannelId())
				.setOngoing(true)
				.setAutoCancel(false)
