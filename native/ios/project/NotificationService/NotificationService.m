...
//    self.bestAttemptContent.title = [NSString stringWithFormat:@"%@ [modified]", self.bestAttemptContent.title];
    ...
    Keychain *keychain = [[Keychain alloc] initWithService:@"SKIMPAppStoreService" withGroup:[NSString stringWithFormat:@"%@%@", bundleSeedId, @"SKIMemberStoreInfo"]];
    ...
    if (ext)
    {
        ...
        [[LPSession downloadTaskWithURL:URL completionHandler:^(NSURL *temporaryLocation, NSURLResponse *response, NSError *error) {
            ...
            if (attachmentError != NULL) {
                NSLog(@"Leanplum : Error with the rich push attachment : %@", [attachmentError localizedDescription]);
                self.contentHandler(self.bestAttemptContent);
                return;
            }
